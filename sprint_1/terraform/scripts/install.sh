...
# Устанавливаем terraform и terragrunt ч/з яндекс-зеркала - делаем исполняемыми бинарные файлы'
...
# Установка helm. Ставим ключ и репозиторий:
...
# Установка gitlab-runner. Ставим официальный репозиторий gitlab-runner и перечитываем списки репозиторий
...
# Клонируем репозитории для установки с помощью kubespray k8s кластера
cd /opt/
git clone https://github.com/IlyaPleshachev/kubernetes_setup.git
cd kubernetes_setup/
...
# Даём нужные разрешения и ключи
